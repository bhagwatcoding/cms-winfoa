...
import { IUser, UserRole } from '@/core/db/interfaces';
import { UserStatus, UserRoleLabels } from '@/core/db/enums';
import type { UpdateResponse, DeleteResponse } from '@/shared/types/api';
...
export async function changeUserRoleAction(
  ...
  try {
    ...
    // Update role
    const roleSlug = UserRoleLabels[newRole].toLowerCase();
...
export async function getAllSubdomainStatsAction() {
  try {
    ...
    const subdomainStats = roleStats.reduce(
      (
        acc: Record<string, SubdomainStats>,
        stat: { _id: string; count: number; activeCount: number; inactiveCount: number }
      ) => {
