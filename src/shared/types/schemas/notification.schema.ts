...
export const notificationChannelSchema = z.enum(['email', 'push', 'sms', 'in_app', 'webhook']);
...
export const bulkNotificationSchema = z.object({
  ...
  scheduledFor: z.date().optional(),
  expiresAt: z.date().optional(),
  sendInBatches: z.boolean().default(true),
  batchSize: z
    .number()
...
export const notificationFilterSchema = z.object({
  ...
  type: z.array(notificationTypeSchema).max(8, 'Too many notification types').optional(),
  status: z.array(notificationStatusSchema).max(6, 'Too many status filters').optional(),
  channel: z.array(notificationChannelSchema).max(4, 'Too many channel filters').optional(),
