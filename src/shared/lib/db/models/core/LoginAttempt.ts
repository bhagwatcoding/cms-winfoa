...
LoginAttemptSchema.statics.record = async function (params: {
    ...
    const recentFailures = await this.getFailedAttempts(
        params.email,
    ...
    const ipFailures = await this.getFailedAttemptsFromIp(
        params.ipAddress,
...
LoginAttemptSchema.statics.isIpBlocked = async function (
    ...
    const recentFailures = await this.getFailedAttemptsFromIp(
        ipAddress,
        new Date(Date.now() - 15 * 60 * 1000)
    );
