...
RateLimitRecordSchema.statics.check = async function (params: {
    ...
    const record = await this.increment(key, type, windowMs);
...
RateLimitRecordSchema.statics.isBlocked = async function (
    key: string
): Promise<boolean> {
...
RateLimitRecordSchema.statics.unblock = async function (
    key: string
): Promise<void> {
    await this.findOneAndUpdate(
        { key },
...
RateLimitRecordSchema.statics.getStats = async function (
    type?: RateLimitType
): Promise<{
    total: number;
    blocked: number;
...
export default (models.RateLimitRecord as IRateLimitRecordModel) ||
    model<IRateLimitRecord, IRateLimitRecordModel>(
        "RateLimitRecord",
