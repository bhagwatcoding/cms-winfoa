...
import { UserStatus, type IUser } from '@/core/db/interfaces';
import type { UpdateResponse } from '@/shared/types/api';
...
export class UMPService {
  ...
  static async getUserManagementData(
    ...
    try {
      ...
      // @ts-expect-error - Permission string type mismatch
      requirePermission(currentUser.userId, 'users:read');
   ...
   * Assign role to user
   */
  static async assignRoleToUser(userId: string, roleId: string): Promise<UpdateResponse<IUser>> {
    try {
      await connectDB();
