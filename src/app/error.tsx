  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-lg text-center">
        <div className="bg-white/95 backdrop-blur-sm rounded-3xl shadow-xl border border-slate-200/50 p-8 sm:p-12">
          ...
          {process.env.NODE_ENV === 'development' && error.digest && (
            <div className="mb-6 p-4 bg-slate-100 rounded-lg text-left">
              <p className="text-xs font-mono text-slate-600 break-all">Error ID: {error.digest}</p>
            </div>
          )}

          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button onClick={reset} className="w-full sm:w-auto gap-2 bg-red-600 hover:bg-red-700">
        ...
        <div className="mt-8 text-white/80 text-sm">WINFOA Education Portal</div>
      </div>
    </div>
  );
}
