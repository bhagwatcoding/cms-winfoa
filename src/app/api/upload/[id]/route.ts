...
import { deleteFile, getFileInfo } from '@/core/upload';
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: "Invalid action. Use 'info' or 'serve'" }, { status: 400 });
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Log deletion
    console.log(
      `🗑️ File(s) deleted: ${deleted} files, ${failed} failed - ${id} by ${currentUser.email}`
    );
