...
export async function POST(request: NextRequest) {
  try {
    ...
    const { type, recipient, template, variables = {}, _sendImmediately = true } = body;
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { recipients, template, variables = {}, _batchSize = 10 } = body;
    ...
    console.log(
      `📧 Bulk notification sent: ${template} to ${recipients.length} recipients by ${currentUser.email} (${result.sent} sent, ${result.failed} failed)`
    );
