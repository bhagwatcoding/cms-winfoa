...
import { ArrowUpRight, ArrowDownLeft, Plus, Wallet, Clock, CreditCard } from 'lucide-react';
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div>
                ...
                <p className="text-slate-500 dark:text-slate-400">Welcome back, manage your finances securely.</p>
            </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <ActionBtn icon={<CreditCard className="text-pink-500" />} label="Pay Bills" color="bg-pink-50 dark:bg-pink-900/20" href="/wallet/bills" />
                            <ActionBtn icon={<ArrowUpRight className="text-emerald-500" />} label="Transfer" color="bg-emerald-50 dark:bg-emerald-900/20" href="/wallet/transfer" />
                            <ActionBtn icon={<Clock className="text-orange-500" />} label="Withdraw" color="bg-orange-50 dark:bg-orange-900/20" href="/wallet/withdraw" />
                            <ActionBtn icon={<Wallet className="text-blue-500" />} label="Cards" color="bg-blue-50 dark:bg-blue-900/20" href="/wallet" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                    ...
                    <Link href="/wallet/history" className="text-sm text-indigo-600 hover:text-indigo-700 font-medium">View All</Link>
                </div>
                            ...
                            <div key={tx._id.toString()} className="p-4 sm:p-6 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500">{new Date(tx.createdAt).toLocaleDateString()} • {new Date(tx.createdAt).toLocaleTimeString()}</p>
                                    </div>
                                </div>
...
function ActionBtn({ icon, label, color, href }: { icon: React.ReactNode, label: string, color: string, href: string }) {
    return (
        <Link href={href} className={`${color} p-3 rounded-xl flex flex-col items-center justify-center gap-2 hover:scale-105 transition-transform duration-200`}>
            {icon}
