...
import { Badge, Button, Card, CardContent, CardDescription, CardHeader } from '@/ui';
import { Key, Plus, Copy, Eye, EyeOff, MoreHorizontal, Activity } from 'lucide-react';
  ...
  },
  {
    ...
    render: (key) => (
      <div className="flex items-center gap-1.5">
        ...
        <span className="font-mono text-sm">{key.requestCount.toLocaleString()}</span>
      </div>
    ),
  },
  {
    key: 'isActive',
    header: 'Status',
    render: (key) => <StatusBadge status={key.isActive ? 'active' : 'inactive'} />,
  },
  {
    key: 'actions',
    header: '',
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage API access tokens and monitor usage</p>
        </div>
