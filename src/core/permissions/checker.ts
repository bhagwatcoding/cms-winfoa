...
export const userHasAnyPermission = cache(
  (user: UserContext | null, permissions: Permission[]): boolean => {
    if (!user) return false;
...
export function getUserPermissions(user: UserContext): Permission[] {
  ...
  if (user.roleId && typeof user.roleId === 'object' && 'permissions' in user.roleId) {
    rolePermissions = (user.roleId as { permissions: Permission[] }).permissions || [];
  }
 ...
 * @throws Error if not authorized
 */
export function requireAnyPermission(user: UserContext | null, permissions: Permission[]): void {
 ...
 * @throws Error if not authorized
 */
export function requireAllPermissions(user: UserContext | null, permissions: Permission[]): void {
  if (!user) {
    throw new Error('Unauthorized - Please login');
  }
