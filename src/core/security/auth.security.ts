 ...
 */
export function createSessionSignature(userId: string, sessionId: string, token: string): string {
  const data = `${userId}:${sessionId}:${token}`;
  return crypto.createHmac('sha256', SESSION.SECRET).update(data).digest('hex');
}
