...
// @ts-expect-error - Mongoose statics typing is complex
SystemSettingSchema.statics.get = async function <T = unknown>(
  this: Model<ISystemSetting>,
  key: string
): Promise<T | null> {
  const setting = (await this.findOne({ key })) as ISystemSetting | null;
...
SystemSettingSchema.statics.reset = async function (key: string): Promise<ISystemSetting | null> {
...
SystemSettingSchema.statics.seedDefaults = async function (): Promise<void> {
  ...
  for (const setting of defaults) {
    await this.findOneAndUpdate({ key: setting.key }, setting, {
      upsert: true,
      new: true,
    });
  }
