...
import { Shield, Mail, User, Settings, LogIn, LogOut, Key, Trash2 } from 'lucide-react';
  ...
  if (!activities || activities.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-muted-foreground mt-2">Your account activity will appear here</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Your account activity from the last 90 days</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {activity.ipAddress && (
                  <p className="text-xs text-muted-foreground mt-1">IP: {activity.ipAddress}</p>
                )}
              </div>

              <div className="text-xs text-muted-foreground">{timestamp.toLocaleString()}</div>
            </div>
          );
        })}
      </div>
