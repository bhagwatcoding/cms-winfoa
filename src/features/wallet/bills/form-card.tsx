...
import { payBill } from '@/apps/wallet/actions';
  ...
  const handlePayBill = async () => {
    ...
    try {
      const result = await payBill(amount, {
        ...
        return { error: 'Something went wrong. Please try again.' }; // Return a consistent error structure
      });
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-12 text-center animate-in fade-in zoom-in duration-500">
        ...
        <p className="mt-2 text-muted-foreground">
          ₹{amount.toLocaleString('en-IN')} has been paid for your {selectedCategoryData?.name}{' '}
          bill.
        </p>
    ...
    <div className="mx-auto max-w-lg space-y-6 px-4 py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Pay all your utility bills in one place</p>
        </div>
      </div>

      {/* Category Selection */}
      <FormCard title="Select Category" description="Choose the type of bill to pay" icon={Receipt}>
