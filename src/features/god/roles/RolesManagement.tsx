...
import { Shield, Plus, Check, ChevronDown, ChevronRight, Trash2, Edit } from 'lucide-react';
        ...
        {initialRoles.map((role) => (
          <Card key={role._id.toString()} className="overflow-hidden">
            <CardHeader
              ...
              onClick={() =>
                setExpandedRole(expandedRole === role._id.toString() ? null : role._id.toString())
              }
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <Badge variant={role.isActive ? 'default' : 'secondary'} className="capitalize">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      {Object.entries(PERMISSION_GROUPS).map(([group, permissions]) => {
                        const groupPermissions = role.permissions.filter((p) =>
                          (permissions as unknown as string[]).includes(p)
                        );
                        if (groupPermissions.length === 0) return null;

                        return (
                          <div key={group} className="rounded-lg border p-3 bg-background">
                            <h5 className="mb-2 font-medium text-xs text-muted-foreground uppercase tracking-wider">
                              {group}
                            </h5>
                            <div className="space-y-1">
                              {groupPermissions.map((permission) => (
                                <div key={permission} className="flex items-center gap-2 text-sm">
                                  ...
                                  <span className="capitalize">{permission.split(':')[1]}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    {!role.isSystem && (
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => handleDelete(role._id.toString())}
                      >
