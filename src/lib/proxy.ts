...
type SubdomainType = 'ump' | 'god' | 'center' | 'api' | 'auth' | 'myaccount' | 'www' | null;
...
export class SubdomainProxy {
    ...
    private static getSubdomain(hostname: string): SubdomainType {
        ...
        if (cleanHost.endsWith(`.${cleanRoot}`)) {
            ...
            // Validate subdomain
            const validSubdomains: SubdomainType[] = ['god', 'center', 'api', 'auth', 'myaccount', 'www'];
    ...
    /**
     * Handle CENTER subdomain (center.example.com)
     * Education center portal
     */
    private static handleCenterSubdomain(
        request: NextRequest,
        path: string,
        isAuthenticated: boolean
    ): NextResponse {
        ...
        // Rewrite to center routes (mapped to education folder)
        return NextResponse.rewrite(new URL(`/education${path === '/' ? '' : path}`, request.url));
    }
    ...
    public static async execute(request: NextRequest): Promise<NextResponse> {
        ...
        switch (subdomain) {
            ...
            case 'center':
                response = this.handleCenterSubdomain(request, path, isAuthenticated);
                break;
