...
module.exports = {
  ...
  deploy: {
    production: {
      ...
      path: '/var/www/winfoa',
      'post-deploy':
        'npm install && npm run build:optimize && pm2 reload ecosystem.config.js --env production',
    ...
    staging: {
      ...
      'post-deploy':
        'npm install && npm run build:optimize && pm2 reload ecosystem.config.js --env staging',
      'pre-setup': 'apt-get update && apt-get install -y git',
      'post-setup': 'ls -la',
      env: {
        NODE_ENV: 'staging',
